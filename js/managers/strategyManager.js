const StrategyManager = {
  ...
  tree: function(playerIndex, players, values, width, height, maxDepth = 4) {
    ...
    let root = { depth: 0, player: playerIndex, activePlayer: playerIndex, score: players[referencePlayerIndex].moves.length, moves: players[referencePlayerIndex].moves, players, values, children: [] };
    let fringe = [ root ];
    ...
    while (node) {
      if (node.depth <= maxDepth && node.moves.length) {
        ...
        node.moves.forEach(move => {
          ...
          const child = { depth: node.depth + 1, player: playerIndex, activePlayer: newPlayerIndex, score: movesReferencePlayer.length, moves, players: newPlayers, values: newValues, children: [] };
  ...
  renderTree: function(tree) {
    ...
    while (node) {
      ...
      ReactDOM.render(
        <div>
          <Isolation width="3" height="3" strategy={ StrategyManager.random } playerIndex = { node.tree.activePlayer } player1x={ player1.x } player1y={ player1.y } player2x={ player2.x } player2y={ player2.y } grid={ node.tree.values } moves={ node.tree.score } cellStyle="small"></Isolation>
        </div>,
